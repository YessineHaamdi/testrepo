...
export class ListChambreBlocComponent {
  ...
  getAllBloc() {
    this.blocService.getAllBlocs().subscribe((res: Bloc[]) => {
      ...
      this.listBlocs.forEach((bloc) => {
        this.foyerService.getFoyerByIdBloc(bloc.idBloc).subscribe((foyer) => {
          bloc.foyer = foyer;
        });
      });
    });
  ...
  affecterBlocAFoyer() {
    if (this.selectedFoyerId !== null && this.selectedBlocId !== null) {
      this.blocService
        .affecterBlocAFoyer(this.selectedBlocId, this.selectedFoyerId)
        .subscribe((res: any) => {
  ...
  affecterChambresABloc() {
    if (this.selectedChambresIds.length > 0 && this.selectedBlocId !== null) {
      this.blocService
        .affecterChambresABloc(this.selectedChambresIds, this.selectedBlocId)
        .subscribe({
          next: (res) => {
            Swal.fire({
              toast: true,
    ...
    this.chambreService.getAllChambres().subscribe((res: Chambre[]) => {
      ...
      this.listChambres.forEach((chambre) => {
        if (chambre.bloc && chambre.bloc.idBloc) {
          this.blocService
            .getBlocById(chambre.bloc.idBloc)
            .subscribe((bloc) => {
              chambre.bloc = bloc;
              if (bloc && bloc.idBloc) {
                this.foyerService
                  .getFoyerByIdBloc(bloc.idBloc)
                  .subscribe((foyer) => {
          ...
          console.log(
            'Aucun bloc associé ou idBloc est null pour la chambre',
            chambre,
          );
        }
      });
